...
btn.addEventListener("click", async () => {
  resultado.innerHTML = `<div class="card"><p class="muted">Testando Supabase...</p></div>`;
  ...
  try {
    ...
    const lojaTeste = await valisysDB.criarLoja({
      nome: `Teste Supabase ${new Date().toLocaleTimeString("pt-BR")}`,
    ...
    resultado.innerHTML = `
      <div class="card">
        <h2>Conexão OK</h2>
        <p>O site conseguiu ler e gravar no Supabase.</p>
        ...
        <p class="muted">Agora abra o Supabase > Table Editor > lojas e veja se essa loja apareceu.</p>
      </div>
    `;
  } catch (erro) {
    console.error(erro);
    ...
    resultado.innerHTML = `
      <div class="card">
        ...
        <p>O site não conseguiu conectar/gravar no Supabase.</p>
        <p><strong>Erro:</strong> ${esc(erro.message)}</p>
        <p class="muted">
          Confira js/supabase-config.js, se o SQL foi rodado e se as policies/RLS não estão bloqueando insert/select.
        </p>
      </div>
    `;
  }
